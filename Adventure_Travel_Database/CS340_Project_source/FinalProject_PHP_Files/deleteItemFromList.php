...
if(!($stmt = $mysqli->prepare("DELETE FROM item WHERE item.description = ? AND item.lid = ?"))){
...
if(!($stmt->bind_param("si",$_POST['itemDescription'],$_POST['listID']))){
...
//should successfully add one row to bsg_people.
	...
	echo "Removed " . $stmt->affected_rows . " item.";
}
